irt_mirt <- function(data,
  ...
  out$model$engine$call <- glue::glue(gsub(
    "tol", "{tol}",
    gsub(
      "item_type", '"{item_type}"',
  ...
  if(abs_fit){
    if (nrow(na.omit(data)) != nrow(data)){
      message ("Missing data removed rowise to estimate absolute fit statistics:")
      out$estimation$abs_fit <- mirt::M2(mirt_model, na.rm = T)
    } else {
  ...
  out$parameters$coefficients <- data.frame(mirt::coef(mirt_model, simplify = T, IRTpars = irt_pars)$items)[, c(2, 1, 3)] %>%
    `colnames<-`(c("difficulty", "discrimination", "guessing")) %>%
    tibble::rownames_to_column("item") %>%
    tibble::tibble()

  out$parameters$persons <- dplyr::rename(tibble::as_tibble(mirt::fscores(mirt_model, full.scores.SE = T)),
                                          ability = "F1", std_err = "SE_F1") %>%
