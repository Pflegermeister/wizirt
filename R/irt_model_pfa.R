...
irt_model_pfa <- function(wizirt_fit, pfa = NULL, predictors = NULL, bins = 5) {
  ...
  for (i in pfa$spec$stats) {
    mod_list[[i]] <- eval(parse(text = glue::glue(
      "blme::blmer(",
      "{i} ~",
      "(1|ids) + .,",
      "data = mlm_data)"
...
pfa_fit_subset <- function(bin, data = mlm_data, stats = pfa$spec$stats) {
  ...
  for (i in stats) {
    fit <- eval(parse(text = glue::glue(
      "PerFit::{i}(mlm_sub %>% dplyr::select(-ability),",
...
get_bins <- function(wizirt_fit, bins) {
  ...
  items <- wizirt_fit$fit$parameters$coefficients %>%
    dplyr::arrange(sample(seq_len(nrow(.)))) %>%
    dplyr::pull(item)
  data.frame(
    item = items,
